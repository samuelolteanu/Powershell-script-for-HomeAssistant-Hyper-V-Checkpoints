...
if (Test-Path $versionFilePath) {
    ...
    $snapshotName = "hassagent manual - {0:dd.MM.yyyy - HH:mm:ss} - ha core v.{1}" -f (Get-Date), $versionNumber
} else {
    $snapshotName = "hassagent manual - {0:dd.MM.yyyy - HH:mm:ss}" -f (Get-Date)
}
